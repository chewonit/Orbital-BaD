...
function login_radius_login_script(){
	...
	if($loginRadiusObject->loginradiusValidateKey($loginRadiusSettings['LoginRadius_apikey'])){
		...
		<!-- Script to enable social login -->
		 <!--<script src="//hub.loginradius.com/include/js/LoginRadius.js"></script>-->		 		 <script src="<?php echo plugins_url(); ?>/loginradius-for-wordpress/js/LoginRadius.1.0.js"></script>
	...
	if(!is_user_logged_in()){
		if($newInterface){ 
			...
			if(trim($loginRadiusSettings['LoginRadius_apikey']) != "" && trim($loginRadiusSettings['LoginRadius_secret']) != ""){
				$result .= "<label><h5 style='margin:3px !important;'>".$title."</h5></label>";
			}
			...
			?> 
			<div>
			<div style='margin-bottom: 3px;'><?php if(trim($loginRadiusSettings['LoginRadius_apikey']) != "" && trim($loginRadiusSettings['LoginRadius_secret']) != ""){?><label><h5 style='margin:3px !important;'><?php _e( $title, 'LoginRadius' ) ?></h5></label><?php } ?></div>
			<?php
